...
A drop-in Claude Code statusline script that shows your rate limit usage — the one thing the built-in statusline doesn't give you.

```
Opus  ctx [⠿⠄⠀⠀⠀] 25%  5h [⠿⠀⠀⠀⠀] 21% ⟳ 5:59pm  7d [⠀⠀⠀⠀⠀] 0% ⟳ mar 15
```

## What it does

- Shows your **5-hour** and **7-day** plan quotas as color-coded braille dot bars (green → yellow → red)
- Shows **reset times** so you know when your limits refill
...
| `--reset` | Reset time for quotas (use with `--quota`) |
...
3. Calls `https://api.anthropic.com/api/oauth/usage` to get your 5-hour and 7-day utilization
4. Caches the response for 60 seconds
5. Prints everything in one line with color-coded braille dot bars
